...
#if defined(CONFIG_ASURADA_MODIFIER_OS_MAC)
static const char *texts[MOD_TYPE_COUNT] = {
    "CMD", "OPT", "CTRL", "SHFT"
};
#elif defined(CONFIG_ASURADA_MODIFIER_OS_WINDOWS)
static const char *texts[MOD_TYPE_COUNT] = {
    "WIN", "ALT", "CTRL", "SHFT"
};
#else
...
#ifdef CONFIG_ASURADA_MODIFIER_ORDER
    const char *config_str = CONFIG_ASURADA_MODIFIER_ORDER;
...
bool modifier_order_uses_symbols(void) {
    return IS_ENABLED(CONFIG_ASURADA_MODIFIER_OS_MAC);
}

bool modifier_order_is_windows(void) {
    return IS_ENABLED(CONFIG_ASURADA_MODIFIER_OS_WINDOWS);
}
