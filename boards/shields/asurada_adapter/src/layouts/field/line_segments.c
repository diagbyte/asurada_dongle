...
static void lines_update(void) {
    ...
    const uint32_t INTENSITY_DECAY_MS = CONFIG_ASURADA_ANIMATION_INTENSITY_DECAY_SEC * 1000;
    const uint32_t FLOW_DECAY_MS = CONFIG_ASURADA_ANIMATION_FLOW_DECAY_SEC * 1000;

    static float intensity_at_stop = 0.0f;
    static float flow_at_stop = 0.0f;

    if (current_wpm > 0 || idle_ms < INTENSITY_DECAY_MS / 10) {
        float speed = ANIM_BASE_SPEED + (current_wpm / (float)CONFIG_ASURADA_ANIMATION_WPM_REFERENCE) * ANIM_WPM_SPEED_MULTIPLIER;
...
static void timer_cb(lv_timer_t *timer) {
    ...
    } else if (animation_started && idle_ms < CONFIG_ASURADA_ANIMATION_FLOW_DECAY_SEC * 1000) {
